...
string a_star4 (vector<string> level, point start, point exit) {

    using vertex2 = tuple<int, vector<point>, string>;

    vertex2 source = {0, {start,start}, ""};
    priority_queue<vertex2,vector<vertex2>, greater<vertex2>> q1;
...
string blox_solver (vector<string> level) {
    ...
    return a_star4 (level, start, exit);
}
...
void Test () {
    ...
    for (int i = 0; i < level.size(); i++) {
        ...
        string path = blox_solver(level[i]);
        if (dotest (path, result[i]) == false) {
            cout << "level " << i + 1 << endl;
