...
std::string evaluate(const std::string &expression, const std::string &number = "") {
    ...
    int index = 0; 
    std::vector<std::string> vars, oper, func;
...
std::string diff(const std::string &t1, const std::string &op, const std::string &t2) {
    ...
    if (op == "+") { // add : a + b => a' + b'
        ...
        return evaluate(operate(dx1, op, dx2));
    } else if (op == "-") { // min : a - b => a' - b'
        std::string dx1 = derivate(t1), dx2 = derivate(t2);
...
std::string derivate(const std::string &input) {
    ...
    while (index < code.size()) {
        ...
        if (code[index] == "x") {
            ...
            /*while (!oper.empty() && order(oper.back()) > order(code[index])) {*/
            /*    std::string t2 = getstack(vars), t1 = getstack(vars), op = getstack(oper);*/
            /*    std::string result = diff(t1,op,t2);*/
            /**/
            /*    std::cout << "[" << t1 << "] " << op << " [" << t2 << "]";*/
            /*    std::cout << " => " << result << "\n";*/
            /**/
