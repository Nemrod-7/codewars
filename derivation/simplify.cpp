...
node *div(node *a, node *b) {
		string gethash(node*);
		//string hash = gethash(a) + gethash(b);

		//cout << "[" << gethash(a) << "]["  << gethash(b)  << "]" << "->" << hash << "\n";
		//if (hash == "x^1x*0") {
		//		if (a->t1->sym == b->sym) {
...
node *mul(node *a, node *b) {
		...
		////cout << "[" << evaluate(a) << "][" << evaluate(b) << "]" << "->" << hash << "\n";
		//if (hash == "1*0x*1") { // (5) * (x * 3)
		//		return mul(mul(a, b->t2), b->t1);
		//} else if (hash == "1*01*x") { // (5) * (3 * x)
		//		return mul(mul(a, b->t1), b->t2);
		//} else if (hash == "1*01/x") { // (5) * (4 / x)
...
int main () {
		...
		node *tree =
				//mul(new node(2.0),
								mul(
										div(new node(3.0), new node("x")),
										exp(new node("x"), new node(3.0))
									 ) 
				//				)
