...
TESTS = (
    ...
    ("More fixed cases", (
        ...
        ('mesh',                                  65,   [ (0, i * 2, 9, i * 2 + 1) for i in range(5) ] + [ (i * 2, 0, i * 2 + 1, 9) for i in range(5) ])
    ))
)
...
def mktest() :
    for title,its in TESTS:
        for subtitle, expect, rect in its:
            assert_equals(calculate(rect), expect)

# for i in range(10) :
#     x1,x2,y1,y2 = [ random.randint(0, 90) for _ in range(4) ]
#     rect = [min(x1,x2), min(y1,y2), max(x1,x2), max(y1,y2)]
#
#     print(rect)

# lim = 100
# graph = [ (0, i * 2, 9, i * 2 + 1) for i in range(lim) ] + [ (i * 2, 0, i * 2 + 1, 9) for i in range(lim) ]
