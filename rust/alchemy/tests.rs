...
mod tests  {
    ...
    mod failure {
        ...
        mod basic_invalid_builds {
            ...
            macro_rules! basic_failure_tests {
                ($( ( $test_name:ident, $message:expr, $branch:expr, $bonds:expr ) ),+) => {
                    $(
                        fn $test_name() {
                            chem_assert!( mol_safe!(branch(&$branch), bond(&$bonds), close()), should err with, ChemError::InvalidBond, $message);
                        }
                     )+
                };
            }
        ...
        mod invalid_mutation_and_addition {
            ...
            failure_tests! {
                ( mutate_full_carbon_1, "Should fail when mutating a carbon with three atoms already linked to an oxygen", [3,1], [(2,1,1,2)] => mutate(&[(2,1,O)])),
                ( mutate_full_carbon_2, "Should fail when mutating a carbon with two double bonds to nitrogen", [3], [(1,1,2,1), (3,1,2,1)] => mutate(&[(2,1,N)])),
                ( add_full_carbon, "Should fail when adding a new hydrogen to a carbon with already 4 bonds", [3], [(1,1,2,1), (3,1,2,1)] => add(&[(2,1,H)])),
                ( overfilling_fails_after_mutating_element, "Should fail when mutating an atom and then adding too many atoms on it", [3], [(1,1,2,1)], mutate(&[(2,1,N)]) => add(&[(2,1,O)])),
                ( chaining_monovalent_atom, "Should fail when chaining atoms after any monovalent atom", [3], [] => add_chain(2, 1, &[C,C,F,H]))
            }
        }
    }
